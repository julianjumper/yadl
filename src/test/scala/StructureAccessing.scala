...
import parser.{StructureAccess, Identifier, YadlInt, StdString, structureAccess}

class StructureAccessing extends munit.FunSuite {
  test("case 'hello[5]'") {
    val input = "hello[5]"
    val expected = StructureAccess(Identifier("hello"), YadlInt(5))
  ...
  test("case 'hello[5][15]'") {
    ...
    val expected = StructureAccess(
      StructureAccess(Identifier("hello"), YadlInt(5)),
      YadlInt(15)
    )
    parse(input, structureAccess(using _)) match {
      case Success(value, index) =>
  ...
  test("case 'hello[index][index]'") {
    val input = "hello[index][index]"
    val expected = StructureAccess(
      StructureAccess(Identifier("hello"), Identifier("index")),
      Identifier("index")
    )
  ...
  test("case 'hello[index][index][index]'") {
    val input = "hello[index][index][index]"
    val expected = StructureAccess(
      StructureAccess(
        StructureAccess(Identifier("hello"), Identifier("index")),
        Identifier("index")
