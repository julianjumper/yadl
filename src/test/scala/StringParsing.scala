...
test("case 8") {
  ...
  val expected = FormatString(List(StdString("Moin "), StdString("{name}"), StdString("!")))
  parse(input, stringP(using _)) match {
    case Success(value, index) =>
      assertEquals(value, expected)
      assertEquals(index, input.length, "input has not been parsed fully")
...
test("case 18") {
  ...
  val expected = FormatString(List(StdString("String with "), StdString("{variable}")))
  parse(input, stringP(using _)) match {
    case Success(value, index) =>
      assertEquals(value, expected)
      assertEquals(index, input.length, "input has not been parsed fully")
...
test("case 22") {
  ...
  val expected = FormatString(List(StdString("{greeting}"), StdString(", "), StdString("{name}"), StdString("!")))
  parse(input, stringP(using _)) match {
    case Success(value, index) =>
      assertEquals(value, expected)
      assertEquals(index, input.length, "input has not been parsed fully")
...
test("case 24") {
  ...
  val expected = FormatString(List(StdString("Formatted multi-line\n"), StdString("{part}"), StdString(" of the string")))
  parse(input, stringP(using _)) match {
    case Success(value, index) =>
      assertEquals(value, expected)
      assertEquals(index, input.length, "input has not been parsed fully")
...
test("case 38") {
  ...
  val expected = FormatString(List(StdString("String with {escaped \\{braces\\}}")))
  parse(input, stringP(using _)) match {
    case Success(value, index) =>
      assertEquals(value, expected)
      assertEquals(index, input.length, "input has not been parsed fully")
...
test("case 41") {
  ...
  val expected = StdString("Multi-line string with indentation\n  and extra spaces")
  parse(input, stringP(using _)) match {
    case Success(value, index) =>
      assertEquals(value, expected)
      assertEquals(index, input.length, "input has not been parsed fully")
